...
ExcelFiller::Workbook::Workbook(
        pugi::xml_node workbook, ExcelFiller::SheetRelations&& relations, SharedStringTable&& sharedStringTable)
    : sharedStringTable_(std::move(sharedStringTable))
{
    for (auto sheet : workbook.child("sheets").children())
    {
        ...
        {
            throw std::runtime_error(fmt::format("Could not find r:id {} in SheetRelations for sheet {}", rid, name));
        }
    }
}
...
std::ostream& ExcelFiller::operator<<(std::ostream& os, const Workbook& wb)
{
    for (const auto& [name, relation] : wb.sheetRelations_)
    {
        os << "Name=" << name << ", Id=" << relation.getId() << ", "
           << "Type=" << static_cast<std::underlying_type_t<SheetRelation::RelationType>>(relation.getType())
