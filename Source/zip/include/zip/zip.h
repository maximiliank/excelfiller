...
#define ZIP_ETDEFLBUF -12   // cannot flush tdefl buffer_
...
#define ZIP_ECAPSIZE -26    // capacity_ size too small
...
/**
 * Closes a zip entry, flushes buffer_ and releases resources.
 *
 * @param zip zip archive handler.
 *
 * @return the return code - 0 on success, negative number (< 0) on error.
...
/**
 * Compresses an input buffer_ for the current zip entry.
 *
 * @param zip zip archive handler.
 * @param buf input buffer_.
 * @param bufsize input buffer_ size (in bytes).
 *
 * @return the return code - 0 on success, negative number (< 0) on error.
 */
...
/**
 * Extracts the current zip entry into output buffer_.
 *
 * The function allocates sufficient memory for a output buffer_.
 *
 * @param zip zip archive handler.
 * @param buf output buffer_.
 * @param bufsize output buffer_ size (in bytes).
 *
 * @note remember to release memory allocated for a output buffer_.
...
/**
 * Extracts the current zip entry into a memory buffer_ using no memory
 * allocation.
 *
 * @param zip zip archive handler.
 * @param buf preallocated output buffer_.
 * @param bufsize output buffer_ size (in bytes).
 *
 * @note ensure supplied output buffer_ is large enough.
 *       zip_entry_size function (returns uncompressed size for the current
 *       entry) can be handy to estimate how big buffer_ is needed.
 *       For large entries, please take a look at zip_entry_extract function.
 *
 * @return the return code - the number of bytes actually read on success.
...
/**
 * Copy zip archive stream output buffer_.
 *
 * @param zip zip archive handler.
 * @param buf output buffer_. User should free buf.
 * @param bufsize output buffer_ size (in bytes).
 *
 * @return copy size
 */
